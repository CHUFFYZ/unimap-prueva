...
try {
    ...
    if (empty($_POST['matriculas'])) {
        throw new Exception("No se seleccionaron administrativos para eliminar");
    }
    ...
    foreach ($matriculas as $matricula) {
        ...
        if ($stmt->fetch()) {
            // Delete admin
            $stmt = $pdo->prepare("DELETE FROM administrativos WHERE matricula = :matricula");
            $stmt->execute([':matricula' => $matricula]);
            $deletedCount++;
        }
    }
    ...
    // Success
    $_SESSION['success'] = "$deletedCount administrativos eliminados exitosamente";
